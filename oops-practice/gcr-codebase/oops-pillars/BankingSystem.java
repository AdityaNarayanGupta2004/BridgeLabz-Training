...
// create a interface name-> Loanable
interface Loanable {
...
class CurrentAccount extends BankAccount implements Loanable {
    ...
    // using a concept of polymorphism (Overridng )
    @Override
    public double calculateInterest() {
        return getBalance() * interestRate;
    }
...
public class BankingSystem {
    public static void main(String[] args) {
        ...
        accounts.add(new SavingsAccount("SAV123", "Mohan", 150000));
        accounts.add(new CurrentAccount("CUR456", "sohan", 300000));

        for (BankAccount acc : accounts) {
            System.out.println("Account Holder Name: " + acc.getHolderName());
            System.out.println("Account Number: " + acc.getAccountNumber());
            System.out.println("Balance : ₹" + acc.getBalance());
            System.out.println("Interest of Loan: ₹" + acc.calculateInterest());
